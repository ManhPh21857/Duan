...
namespace _3.PL.Views
{
    ...
    public partial class FrmChiTietSp : Form
    {
        ...
        public void LoadToData()
        {
            ...
            dtg_show.Columns[3].Name = "Loại SP";
            dtg_show.Columns[4].Name = "Màu Sắc";
            dtg_show.Columns[5].Name = "Hãng giày";
            dtg_show.Columns[6].Name = "Số lượng tồn";
            dtg_show.Columns[7].Name = "Mô tả";
            dtg_show.Columns[8].Name = "Giá nhập";
            dtg_show.Columns[9].Name = "Giá bán";
            dtg_show.Columns[10].Name = "Size";
            dtg_show.Rows.Clear();
            var lst = _iCTSP.GetAllView();
            
            foreach (var item in lst)
            {
                dtg_show.Rows.Add(item.ChiTietSP.Id, stt++, item.SanPham.TenSP,item.LoaiSP.TenLoai,item.MauSac.TenMs,item.HangSP.TenHang , item.ChiTietSP.Soluongton,item.ChiTietSP.Mota ,item.ChiTietSP.GiaNhap, item.ChiTietSP.GiaBan, item.ChiTietSP.Size);
            }
        }
        ...
        {
            var ten = _iCTSP.GetAll().FirstOrDefault(p => p.SanPham.TenSP == cbb_tensp.Text);
        ...
        private void dtg_show_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex >= 0)
            {
                ...
                cbb_loaisp.Text = _LoaiSP.GetAll().FirstOrDefault(p => p.Id == _ChiTietSP.IdHang).TenLoai;
                cbb_mausac.Text = _MauSac.GetAll().FirstOrDefault(p => p.Id == _ChiTietSP.IdHang).TenMs;
                tb_soluongton.Text =dt.Cells[6].Value.ToString();
                tb_mota.Text = dt.Cells[7].Value.ToString();
                tb_gianhap.Text = dt.Cells[8].Value.ToString();
                tb_giaban.Text = dt.Cells[9].Value.ToString();
                tb_size.Text = dt.Cells[10].Value.ToString();
            }
        }
